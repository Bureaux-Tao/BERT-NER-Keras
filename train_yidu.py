...
if __name__ == '__main__':
    # 读取训练集和测试集数据
    input_train, result_train = read_data('./data/%s.train' % event_type)
    input_val, result_val = read_data('./data/%s.validate' % event_type)
    input_test, result_test = read_data('./data/%s.test' % event_type)
    ...
    # 验证集
    input_val_labels, input_val_types = PreProcessInputData(input_val)
    result_val = PreProcessOutputData(result_val)
    # 测试集
    ...
    save_file_path = "./models/{}_{}_ep".format(event_type, BASE_MODEL_DIR)
    callbacks_list = [
        keras.callbacks.History(),
        keras.callbacks.EarlyStopping(monitor = "val_loss", patience = 10, verbose = 1),  # 提前结束
        keras.callbacks.ModelCheckpoint(save_file_path + "{epoch:02d}.h5", monitor = 'val_loss', verbose = 1, save_best_only = False,
                                        mode = 'min', period = 1)
    ]
    ...
    history = model.fit(x = [input_train_labels, input_train_types],
                        ...
                        batch_size = BATCH_SIZE,
                        epochs = EPOCH,
                        validation_data = [[input_val_labels, input_val_types], result_val],
                        verbose = 1,
                        shuffle = True,
